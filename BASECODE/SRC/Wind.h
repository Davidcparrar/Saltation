...
public:
  ...
  int time2Average;
  int size;
  void init(int n, double BS);
  ~WindInfo(void);
  void reset(void);
  friend class Wind;
};

void WindInfo::init(int n, double BS){
  size = n;
  int i;
  ForceX = new double [size];
  ForceY = new double [size];
  ForceEX = new double [size];
  ForceEY = new double [size];
  ...
  ChargeFlux= new double [size];
  MassFlux= new double [size];
  #ifdef BINDIST
...
void Wind::Init(Body *Particles){
  ...
  //Making histogram	
  for(i=0;i<N;i++){
    iy = static_cast<int>(Particles[i].y/(D_MEAN*BS));
    histogram[iy]+=1;
  }
   set = 0;
   height=0;
  ...
  #endif
  for(i=0;i<SIZE;i++){
    windVelocity[i]=WINDVEL*log((i*BS+BS+1.0/30)/(1.0/30))/0.4;
  ...
  for(i=0;i<SIZE;i++){
   ...
   for(i=0;i<N;i++){
    inty[i] =  static_cast<int>(Particles[i].y/(BS*D_MEAN));
  ...
  for(j=0;j<10000;j++){ 
    ...
    for(i=0;i<SIZE;i++){
      ...
      windVelocity[i]=0;
      if(nTau[i]<0)
        dwindVelocity[i]= -1.0*WINDVEL*log(((i+1-newHeight)*BS+1/30.0)/((i-newHeight)*BS+1/30.0))*sqrt(-nTau[i])/0.4;
      else
        dwindVelocity[i]= WINDVEL*log(((i+1-newHeight)*BS+1/30.0)/((i-newHeight)*BS+1/30.0))*sqrt(nTau[i])/0.4;
    ...
    for(k=0;k<N;k++){
      Forcex[k]=0;
      VelAir=log(((Particles[k].y/(BS*D_MEAN)-newHeight)*BS+1/30.0)/((inty[k]-newHeight)*BS+1/30.0))/log(((inty[k]+1-newHeight)*BS+1/30.0)/((inty[k]-newHeight)*BS+1/30.0))*(windVelocity[inty[k]+1]-windVelocity[inty[k]])+windVelocity[inty[k]];
  ...
  for(i=0;i<SIZE;i++){
    finfo<<fixed<<(i+1.0/30)*LENGTH*BS<<"\t"<<windVelocity[i]*LENGTH/TIME<<"\t"<<previousWindVelocity[i]*LENGTH/TIME<<endl;
  }
  finfo.close();
  faverage.close();
  #endif
...
void Wind::ComputingWindForces(Body *Particles, double time){
  ...
  for(i=0;i<SIZE;i++){
   ...
   for(i=0;i<N;i++){
    inty[i] = static_cast<int>(Particles[i].y/(D_MEAN*BS));
    Particles[i].Bed=0;
  ...
  for(j=0;j<10;j++){ 
    ...
    for(i=0;i<SIZE;i++){
      ...
      if(i==0)
        ...
        if(nTau[i]<0)
          windVelocity[i]=(1-c)*(-1.0*WINDVEL*log(((i+1-newHeight)*BS+1/30.0)/((i-newHeight)*BS+1/30.0))*sqrt(-nTau[i])/0.4+windVelocity[i-1])+(c)*previousWindVelocity[i];
        else
          windVelocity[i]=(1-c)*(WINDVEL*log(((i+1-newHeight)*BS+1/30.0)/((i-newHeight)*BS+1/30.0))*sqrt(nTau[i])/0.4+windVelocity[i-1])+(c)*previousWindVelocity[i];
      }
    ...
    for(i=0;i<SIZE;i++){
     ...
     for(k=0;k<N;k++){
      Forcex[k]=0;
      VelAir=log(((Particles[k].y/(D_MEAN*BS)-newHeight)*BS+1/30.0)/((inty[k]-newHeight)*BS+1/30.0))/log(((inty[k]+1-newHeight)*BS+1/30.0)/((inty[k]-newHeight)*BS+1/30.0))*(windVelocity[inty[k]+1]-windVelocity[inty[k]])+windVelocity[inty[k]];
     if ((VelAir != VelAir) || (VelAir>fmax(windVelocity[inty[k]+1],windVelocity[inty[k]]))){ 
       VelAir=fmax(windVelocity[inty[k]+1],windVelocity[inty[k]]);
     }
      VelocityAux = Particles[k].Vx - VelAir;
      ...
      Cdrag=Cd(ReAux);
      if(VelocityMag!=0){
        Forcex[k]=-0.5*M_PI*(Particles[k].R)*(Particles[k].R)*AIRDENSITY*Cdrag*VelocityAux*VelocityMag;
        Forcey[k]=-0.5*M_PI*(Particles[k].R)*(Particles[k].R)*AIRDENSITY*Cdrag*VelocityAuy*VelocityMag;
        Forcez[k]=-0.5*M_PI*(Particles[k].R)*(Particles[k].R)*AIRDENSITY*Cdrag*VelocityAuz*VelocityMag;
      }
      TauSingle[inty[k]]+=Forcex[k]*SHEARAREA;
    }
  ...
  for(i=0;i<N;i++){
    ...
    viento.ChargeFlux[inty[i]]+=Particles[i].q*Particles[i].Vx;
    viento.MassFlux[inty[i]]+=Particles[i].m*Particles[i].Vx;
    viento.ForceEX[inty[i]]+=Particles[i].EFx;
    viento.ForceEY[inty[i]]+=Particles[i].EFy;
  }
  height=newHeight;
  contShear++;
...
void Wind::PrintConcentrationProfile(ofstream &fout){
  int i;
  double auxC=0,auxShearX=0,auxShearY=0,auxVelX=0,auxVelY=0,auxCharge=0,auxChargeToMass=0,auxChargeToArea=0,auxMass=0,auxChargeNonAbs=0,auxChargeNonAbs2=0,auxVelProf=0,auxChargeFlux=0,auxMassFlux=0,auxEFx=0,auxEFy=0;
  #ifdef BINDIST
  ...
  for(i=0;i<viento.size;i++){
    ...
    auxChargeFlux+=viento.ChargeFlux[i];
    auxMassFlux+=viento.MassFlux[i];
    auxEFx+=viento.ForceEX[i];
    auxEFy+=viento.ForceEY[i];
    #ifdef BINDIST
    ...
    #ifdef BINDIST
    fout<<scientific<<(i-4)*D_MEAN*LENGTH*BS<<"\t"<<auxC/(viento.time2Average*viento.Box*DEEP*WIDTH)<<"\t"<<auxShearX*MASS*LENGTH/(TIME*TIME*viento.time2Average*viento.Box)<<"\t"<<auxShearY*MASS*LENGTH/(viento.time2Average*viento.Box*TIME*TIME)<<"\t"<<auxVelX*LENGTH/TIME<<"\t"<<auxVelY*LENGTH/TIME<<"\t"<<auxC/(viento.time2Average*viento.Box*DEEP*WIDTH)*auxVelX*LENGTH/TIME<<"\t"<<auxChargeNonAbs*ELEMENTARYCHARGE*AMPERE*TIME<<"\t"<<auxMass*MASS<<"\t"<<auxChargeToArea*ELEMENTARYCHARGE/(LENGTH*LENGTH)*AMPERE*TIME<<"\t"<<part/viento.time2Average<<"\t"<<auxChargeNonAbs2*ELEMENTARYCHARGE/(MASS)*AMPERE*TIME<<"\t"<<auxVelProf*LENGTH/(TIME*viento.time2Average*viento.Box)<<"\t"<<small/viento.time2Average<<"\t"<<big/viento.time2Average<<"\t"<<smallCh/viento.time2Average<<"\t"<<bigCh/viento.time2Average<<"\t"<<ChargeS*ELEMENTARYCHARGE/viento.time2Average<<"\t"<<ChargeB*ELEMENTARYCHARGE/viento.time2Average<<"\t"<<auxChargeFlux*ELEMENTARYCHARGE*LENGTH/viento.time2Average<<"\t"<<auxMassFlux*LENGTH/viento.time2Average<<"\t"<<auxEFx<<"\t"<<auxEFy<<endl;
    #else
    fout<<scientific<<(i-4)*D_MEAN*LENGTH*BS<<"\t"<<auxC/(viento.time2Average*viento.Box*DEEP*WIDTH)<<"\t"<<auxShearX*MASS*LENGTH/(TIME*TIME*viento.time2Average*viento.Box)<<"\t"<<auxShearY*MASS*LENGTH/(viento.time2Average*viento.Box*TIME*TIME)<<"\t"<<auxVelX*LENGTH/TIME<<"\t"<<auxVelY*LENGTH/TIME<<"\t"<<auxC/(viento.time2Average*viento.Box*DEEP*WIDTH)*auxVelX*LENGTH/TIME<<"\t"<<auxChargeNonAbs*ELEMENTARYCHARGE*AMPERE*TIME<<"\t"<<auxMass*MASS<<"\t"<<auxChargeToArea*ELEMENTARYCHARGE/(LENGTH*LENGTH)*AMPERE*TIME<<"\t"<<part/viento.time2Average<<"\t"<<auxChargeNonAbs2*ELEMENTARYCHARGE/(MASS)*AMPERE*TIME<<"\t"<<auxVelProf*LENGTH/(TIME*viento.time2Average*viento.Box)<<"\t"<<auxChargeFlux*ELEMENTARYCHARGE*LENGTH/viento.time2Average<<"\t"<<auxMassFlux*LENGTH/viento.time2Average<<"\t"<<auxEFx<<"\t"<<auxEFy<<endl;
      #endif
      part=0;
      auxC=0;auxShearX=0;auxShearY=0;auxVelX=0;auxVelY=0;auxCharge=0;auxChargeToMass=0;auxChargeToArea=0,auxMass=0,auxChargeNonAbs=0,auxChargeNonAbs2=0;auxVelProf=0,auxChargeFlux=0,auxMassFlux=0;auxEFx=0;auxEFy=0;
      #ifdef BINDIST
