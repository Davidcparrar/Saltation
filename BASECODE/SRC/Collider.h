...
double C11(double Ri, double Rj, double alpha){
  double coef=0;
  for(int i=1;i<20000;i++){
    coef+=1.0/(Rj*sinh(i*alpha)+Ri*sinh((i-1)*alpha));
  }
  return coef;
}

double C22(double Ri, double Rj, double alpha){
  double coef=0;
  for(int i=1;i<5000;i++){
    coef+=1.0/(Ri*sinh(i*alpha)+Rj*sinh((i-1)*alpha));
  }
  return coef;
}

double C121(double Ri, double Rj, double alpha){
  double coef=0;
  for(int i=1;i<5000;i++){
    coef+=1.0/((Ri+Rj+D_BRIDGEK)*sinh(i*alpha));
  }
  return coef;
}

double cosalpha(double Ri, double Rj){
  return acosh(1+(Ri+Rj)/(Ri*Rj)*D_BRIDGEK+D_BRIDGEK*D_BRIDGEK/(2*Ri*Rj));
}
void Collider::BodyVsBody(Body &Bodyi,Body &Bodyj,int i, int j, double Height){
  ...
  if(h>0){
    ...
    if(CollitionsGrain[col].hold==0 && altint*1.0 > 0.4*Height){
      ...
      /*if(Ri<Rj){
        double alpha=cosalpha(Ri,Rj);
        double c11=4*M_PI*E0*Ri*Rj*sinh(alpha)*C11(Ri,Rj,alpha);
        double c12=-4*M_PI*E0*Ri*Rj*sinh(alpha)*C121(Ri,Rj,alpha);
        double c21=-4*M_PI*E0*Ri*Rj*sinh(alpha)*C121(Ri,Rj,alpha);
        double c22=4*M_PI*E0*Ri*Rj*sinh(alpha)*C22(Ri,Rj,alpha);

        double C1=(c11+c12)/(c11+c12+c21+c22);
        double C2=(c11*c22+c12*c21)/(c11+c12+c21+c22);

        double qt= (Bodyj.q+ Bodyi.q)*ELEMENTARYCHARGE;
        double Pot=6/(LENGTH*LENGTH)*(Rj-Ri)/(Rj+Ri);

        Bodyi.q=(C1*(qt)-C2*Pot)/ELEMENTARYCHARGE;
        Bodyj.q=((1-C1)*(qt)+C2*Pot)/ELEMENTARYCHARGE;
        //clog<<Ri<<"\t"<<Rj<<"\t"<<Bodyi.q<<"\t"<<Bodyj.q<<"\t"<<c11<<"\t"<<c22<<"\t"<<c12<<"\t"<<c21<<"\t"<<C1<<"\t"<<C2<<"\t"<<Pot<<"\t"<<alpha<<endl;
        //exit(0);
      }
      else{
        double alpha=cosalpha(Rj,Ri);
        double c11=4*M_PI*E0*Ri*Rj*sinh(alpha)*C11(Rj,Ri,alpha);
        double c12=-4*M_PI*E0*Ri*Rj*sinh(alpha)*C121(Rj,Ri,alpha);
        double c21=-4*M_PI*E0*Ri*Rj*sinh(alpha)*C121(Rj,Ri,alpha);
        double c22=4*M_PI*E0*Ri*Rj*sinh(alpha)*C22(Rj,Ri,alpha);

        double C1=(c11+c12)/(c11+c12+c21+c22);
        double C2=(c11*c22+c12*c21)/(c11+c12+c21+c22);

        double qt= (Bodyj.q+ Bodyi.q)*ELEMENTARYCHARGE;
        double Pot=6/(LENGTH*LENGTH)*(Ri-Rj)/(Rj+Ri);

        Bodyj.q=(C1*(qt)-C2*Pot)/ELEMENTARYCHARGE;
        Bodyi.q=((1-C1)*(qt)+C2*Pot)/ELEMENTARYCHARGE;
        //clog<<Ri<<"\t"<<Rj<<"\t"<<Bodyi.q<<"\t"<<Bodyj.q<<"\t"<<c11<<"\t"<<c22<<"\t"<<c12<<"\t"<<c21<<"\t"<<C1<<"\t"<<C2<<"\t"<<Pot<<"\t"<<alpha<<endl;
        //exit(0);
      }*/
      ...
      Diff_Transfer=(Aj*((Bodyj.H+Bodyj.q)/(4*M_PI*Rj*Rj))-Ai*((Bodyi.H+Bodyi.q)/(4*M_PI*Ri*Ri)))/(2.0)-MOBILITY*D_BRIDGE/(4*M_PI*E0)*(Bodyi.q/(Ri*Ri)-Bodyj.q/(Rj*Rj));

//      if(ions_i>ions_j){

  //      if(Bodyi.getAverageVel()>1e-1/LENGTH && Bodyi.Bed==0)
          Bodyi.IonTransferVolume(Diff_Transfer);
    //    if(Bodyj.getAverageVel()>1e-1/LENGTH && Bodyi.Bed==0)
          Bodyj.IonTransferVolume(-Diff_Transfer);
    ...
    if(Bodyi.q!=0 && Bodyi.Bed==1)
      if(Bodyi.getAverageVel()<1e-1/LENGTH)
        Bodyi.setChargeWall();
    if(Bodyj.q!=0 && Bodyi.Bed==1)
...
void Collider::ChargeVsCharge(Body &Bodyi,Body &Bodyj,int i, int j, double Height){
  ...
  if(Bodyi.Charged!=0 && Bodyj.Charged!=0){
    ...
    Bodyi.EFx-=RefForceijx*qxy*KE;	     Bodyj.EFx+=RefForceijx*qxy*KE;
    Bodyi.EFy-=RefForceijx*qxy*KE;  	     Bodyj.EFy+=RefForceijx*qxy*KE;
    ...
    Bodyi.EFx+=fx;    	     Bodyj.EFx-=fx;
    Bodyi.EFy+=fy;    	     Bodyj.EFy-=fy;
  }
}
    ...
    int k=static_cast<int>(num/Area);
      ...
      for(unsigned int lmn=0;lmn<sizeSA;lmn++){
        ...
        int n=static_cast<int>(num2/Area);
          int GreenX=abs(l-i); int GreenY=abs(m-j); int GreenZ=abs(n-k); int GreenM=m+j;
...
void Collider::ElectricForceReal(Body *Body){
  ...
  for(n=0;n<N;n++){
    if(Body[n].q!=0){
      ...
      Body[n].EFx=Ex[a][b][c]*C1;

      Body[n].Fy+=Ey[a][b][c]*C1;

      Body[n].EFy=Ey[a][b][c]*C1;
