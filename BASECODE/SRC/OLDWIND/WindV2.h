...
void Wind::Init(Body *Particles){
  ...
  //Making histogram	
  for(i=0;i<N;i++){
    iy = static_cast<int>(Particles[i].y/(D_MEAN*0.5));
    histogram[iy]+=1;
  }
   set = 0;
   setted= 0;
  ...
  for(i=0;i<SIZE;i++){
   ...
   for(i=0;i<N;i++){
    inty[i] =  static_cast<int>(Particles[i].y/(D_MEAN*0.5));
  ...
  for(j=0;j<10000;j++){ 
    ...
    for(k=0;k<N;k++){
     ...
     if ((VelAir != VelAir) || (VelAir>fmax(windVelocity[inty[k]+1],windVelocity[inty[k]]))){ 
      ...
      VelocityMag= sqrt(VelocityAux*VelocityAux+Particles[k].Vy*Particles[k].Vy+Particles[k].Vz*Particles[k].Vz);
...
void Wind::PrintConcentrationProfile(ofstream &fout){
  int i;
  double auxC=0,auxShearX=0,auxShearY=0,auxVelX=0,auxVelY=0,auxCharge=0,auxChargeToMass=0,auxChargeToArea=0,auxMass=0,auxChargeNonAbs=0,auxChargeNonAbs2=0;
  int cont = 0;
  int part=0;
  for(i=0;i<viento.size;i++){
    ...
    if(cont%5==0){
     ...
     fout<<scientific<<i-4<<"\t"<<auxC/(viento.time2Average*viento.Box*DEEP*WIDTH)<<"\t"<<auxShearX*MASS*LENGTH/(TIME*TIME*viento.time2Average*viento.Box)<<"\t"<<auxShearY*MASS*LENGTH/(viento.time2Average*viento.Box*TIME*TIME)<<"\t"<<auxVelX*LENGTH/TIME<<"\t"<<auxVelY*LENGTH/TIME<<"\t"<<auxC/(viento.time2Average*viento.Box*DEEP*WIDTH)*auxVelX*LENGTH/TIME<<"\t"<<auxChargeNonAbs*ELEMENTARYCHARGE*AMPERE*TIME<<"\t"<<auxChargeToMass*ELEMENTARYCHARGE/(MASS)*AMPERE*TIME<<"\t"<<auxChargeToArea*ELEMENTARYCHARGE/(LENGTH*LENGTH)*AMPERE*TIME<<"\t"<<auxMass*ELEMENTARYCHARGE/(MASS)*AMPERE*TIME<<"\t"<<auxChargeNonAbs2*ELEMENTARYCHARGE/(MASS)*AMPERE*TIME<<endl;
      part=0;
      auxC=0;auxShearX=0;auxShearY=0;auxVelX=0;auxVelY=0;auxCharge=0;auxChargeToMass=0;auxChargeToArea=0,auxMass=0,auxChargeNonAbs=0,auxChargeNonAbs2=0;
     }
